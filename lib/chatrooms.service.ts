...
export async function getVideoChatById(meetingId: string) {
  if (!meetingId) {
    throw new Error("Invalid meeting ID"); // Guard clause for invalid ID
  }

  const videoChat = await database.getDocument(
    Server.dbId,
    Server.meetingsCollecctionId, // Ensure correct collection ID
    meetingId // Use the correct meeting ID
  );
