...
export async function createStudyRoom(
  ...
  try{
    ...
    console.log('Creating study room:', { name, status, image_url, subject, userId }); 
    ...
    await joinStudyRoom({ study_room_id: studyRoom.$id, user_id: userId, role: "owner" }, userId);
    return studyRoom;
  }
  catch(error){
